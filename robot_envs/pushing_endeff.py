...
class PushingEndeff(gym.Env):
    ...
    def __init__(self, reward_specs, initial_pushee_pos, max_torque=0.1, observable=[], visualize=False, exp_name='', output_dir='', initial_joint_state=None, fixed_timestep=None, full_log=False):
        ...
        robot = p.loadMJCF(os.path.join(os.path.dirname(__file__), 'pushing_endeff.xml'))
    ...
    def init_reward(self, rewards_config):
        ...
        for reward_type, reward_spec in rewards_config.items():
            ...
            elif reward_type == 'pushing_incentive':
                self.reward_parts[reward_type] = PushingIncentive(self.get_endeff_pos, self.get_pushee_pos, reward_spec['goal_pos'], reward_spec['box_dim'], reward_spec['k_i'], reward_spec['angle_incentive'])
            else:
...
if __name__ == "__main__":
    pushing_arm = PushingEndeff(reward_specs={}, initial_pushee_pos=[-0.15, 0.15], observable=['joint_loads', 'endeff_force', 'pushee_state'], visualize=True, max_torque=0.05,
