...
from baselines.ppo1_primitives.mlp_policy import MlpPolicy as PPOMlpPolicy
from baselines.ppo1_base.mlp_policy import MlpPolicy as PPOBaseMlpPolicy
...
class PPOActorWrapper():
    ...
    def policy_fn(name, ob_space, ac_space, exp_config):
        pi_for_shared = PPOMlpPolicy(name + '_for_reuse',
                                  ob_space=ob_space,
                                  ac_space=ac_space,
                                  num_hid_layers=2,
...
class PPOBaseActorWrapper():
    ...
    def policy_fn(name, ob_space, ac_space, exp_config):
        ...
        # TODO: temp, there was a bug in last training
        hid_size = 64
        num_hid_layers = 2

        pi = PPOBaseMlpPolicy(name=name,
                       ob_space=ob_space,
                       ac_space=ac_space,
                       hid_size=hid_size,
    ...
    def __init__(self, ob_space, ac_space, exp_folder):
        ...
        self.pi = PPOBaseActorWrapper.policy_fn("pi", ob_space, ac_space, conf)
...
def run_episode(
    ...
    without_stopping=False,
    states=None,
    termination_conf=None,
    action_filter_conf=None,
    initialization_conf=None,
    use_movable_disk_surface=None,
    ...
    #################### BOLT CODE UPDATE CONVERSIONS ####################
    # DONE: Check this handling of conversion from old reward threshold termination to the new one.
    if 'termination_reward_threshold' in env_params:
        threshold = env_params['termination_reward_threshold']
        del env_params['termination_reward_threshold']
        reward_part = None
        if 'termination_reward' in env_params:
            reward_part = env_params['termination_reward']
            del env_params['termination_reward']
        termination_conf = {'reward_threshold_termination':
                                {'threshold': threshold, 'reward_part': reward_part}}

    if 'init_traj_file' in env_params:
        env_params['demo_traj_file'] = env_params['init_traj_file']
        env_params['initialization_conf'] = {'on_demo_traj': True}
        del env_params['init_traj_file']

    # if 'termination_conf' in env_params:
    #     if 'foot_placement_termination' in env_params['termination_conf']:
    #         del env_params['termination_conf']['foot_placement_termination']
    ################################################################################
    ...
    if termination_conf is not None:
        env_params['termination_conf'] = termination_conf
    if action_filter_conf is not None:
        env_params['action_filter_conf'] = action_filter_conf
    if initialization_conf is not None:
        env_params['initialization_conf'] = initialization_conf
    if use_movable_disk_surface is not None:
        env_params['use_movable_disk_surface'] = use_movable_disk_surface
    if movable_disks_surface_conf is not None:
        env_params['movable_disks_surface_conf'] = movable_disks_surface_conf
    if log_conf is not None:
        env_params['log_conf'] = log_conf
