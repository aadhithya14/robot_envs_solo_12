...
def plot_rewards(i):
    ...
    latest_log = max(os.listdir(folder + str(i).zfill(3) + '/val_env_episodes/'))
    log_file = folder + str(i).zfill(3) + '/val_env_episodes/' + latest_log
    plot_file = out_folder + str(i).zfill(3) + '_rew.png'
    ...
    circle_kt = conf['env_params'][0]['env_specific_params']['reward_specs']['circle']['k_t']
    pos_reward = circle_kt * np.array(log['pos_reward'])[::2]
    vel_reward = circle_kt * np.array(log['vel_reward'])[::2]
    elbow_penalty = np.array(log['elbow_penalty'])
    orientation = np.array(log['orientation'])
    circle = np.array(log['circle'])
    total_reward = pos_reward + vel_reward + elbow_penalty + orientation
    ...
    axes[0, 0].plot(pos_reward)
    axes[0, 1].plot(vel_reward)
    axes[1, 0].plot(elbow_penalty)
    axes[1, 1].plot(orientation)
